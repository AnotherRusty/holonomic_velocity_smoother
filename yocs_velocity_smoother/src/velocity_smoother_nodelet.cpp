...
void VelocitySmoother::velocityCB(const geometry_msgs::Twist::ConstPtr& msg)
{
  ...
  if (holonomic)
  {
    incoming_cmd_vel = *msg;
    geometry_msgs::Twist composed_vel;
    composed_vel.linear.x = sqrt(pow(msg->linear.x, 2) + pow(msg->linear.y, 2));
    target_vel.linear.x  =
          composed_vel.linear.x > 0.0 ? std::min(composed_vel.linear.x,  speed_lim_v) : std::max(msg->linear.x, -speed_lim_v);
...
void VelocitySmoother::robotVelCB(const geometry_msgs::Twist::ConstPtr& msg)
{
  if (robot_feedback == COMMANDS)
  {
    if (holonomic)
    {
      ...
      current_vel.linear.x = sqrt(pow(msg->linear.x, 2) + pow(msg->linear.y, 2));
      current_vel.angular.z = msg->angular.z;
    }
    else
    {
      current_vel = *msg;
...
void VelocitySmoother::spin()
{
  ...
  while (! shutdown_req && ros::ok())
  {
    ...
    if (holonomic)
    {
      ...
      geometry_msgs::Twist decomposed_vel;
      decomposed_vel.linear.x = cmd_vel->linear.x*incoming_cmd_vel.linear.x/(sqrt(pow(incoming_cmd_vel.linear.x, 2)
            +pow(incoming_cmd_vel.linear.y, 2)));
      decomposed_vel.linear.y = cmd_vel->linear.x*incoming_cmd_vel.linear.y/(sqrt(pow(incoming_cmd_vel.linear.x, 2)
            +pow(incoming_cmd_vel.linear.y, 2)));
      
      cmd_vel->linear.x = decomposed_vel.linear.x;
      cmd_vel->linear.y = decomposed_vel.linear.y;

      holonomic_smooth_vel_pub.publish(cmd_vel);
    }
    else
    {
      smooth_vel_pub.publish(cmd_vel);
...
bool VelocitySmoother::init(ros::NodeHandle& nh)
{
  ...
  // to support holonomic robots
  nh.param("holonomic",      holonomic,     false);
  ...
  // to support holonomic robots
  holonomic_smooth_vel_pub = nh.advertise <geometry_msgs::Twist> ("holonomic_smooth_cmd_vel", 1);
